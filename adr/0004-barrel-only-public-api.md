...
`pull()` still exists because the barrel needs something to call at runtime to resolve a Pin against the Manifest and construct a `Compiled template`. But that runtime call is hidden inside generated code.
